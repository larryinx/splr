...
"""
Pre-training GPT-2 from scratch for causal language modeling on a text dataset.
Based on HuggingFace run_clm.py.
"""
...
class DataTrainingArguments:
    ...
    block_size: Optional[int] = field(
        default=None,
        metadata={"help": "Optional input sequence length after tokenization. The training dataset will be truncated in block of this size for training."},
    )
...
def main():
    ...
    def tokenize_function(examples):
        # Append EOS token to each sample so the model learns sample boundaries
        texts = [text + tokenizer.eos_token for text in examples[text_column_name]]
