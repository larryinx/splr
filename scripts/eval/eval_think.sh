#!/bin/bash
# Evaluation script for the SPLR model (reasoning-only / think mode).
...
EVAL_RESULTS_DIR="$PROJECT_DIR/results/experiments/eval_results_think/${ARCH}_${MODE}_${MAX_REASONING_STEPS}"
...
torchrun \
    ...
    main.py \
    --config-name cfg_eval_think \
    arch=$ARCH \
    input_mode=$MODE \
    tokenizer_path=$TOKENIZER_PATH \
    project_name=splr_think_eval \
